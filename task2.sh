...
OUTPUT=${1%.*}".json"
function header(){
  re='\[\s*(.*\s*.*) \].*1..([0-9]+)'  # [ Asserts Samples ], 1..7 tests
  if [[ $line =~ $re ]]; then
...
function body(){
  re='^(ok|not ok).*\b([0-9]+)\s+(.*),.*\b([0-9]+)ms'  # ok  3  expecting command fails (the same as above, bats way), 23ms
  if [[ $line =~ $re ]]; then
    num+=("${BASH_REMATCH[2]}")
...
function overall(){
 re='([0-9]+).*,\s+([0-9]+).*,.*\s+([0-9]+(\.[0-9]+)?)%,.* ([0-9]+)ms'  # 5 (of 7) tests passed, 2 tests failed, rated as 71.43%, spent 136ms
 if [[ $line =~ $re ]]; then
